...
Tr_in = 1/2 .*xi_v_in.^2
Tr_out = 1/2 .*xi_v_out.^2
U1 = 1/2 *m1 *ω_z1^2 .*sol[1,:].^2 + V0/2/eta0^2 *Q1.*sol[1,:].^2 .*cos.((ω_zin-ω_zout).*sol.t).-1/2 *m1* ω_z1^2*z1_eq^2
U2 = 1/2 *m2 *ω_z2^2 .*sol[5,:].^2 + V0/2/eta0^2 *Q2.*sol[5,:].^2 .*cos.((ω_zin-ω_zout).*sol.t).-1/2 *m2* ω_z2^2*z2_eq^2
plot(sol.t,[(Tz_in.+Tz_out.+U1.+U2)./(kb*T)])
#p1 = plot(sol.t,[(Tz_in.+Tz_out.+1/2 m1*ω_z1^2 .*sol[1,:].^2 .+1/2 m2*ω_z2^2 .*sol[5,:].^2), label = L"T_{in,z}")
# p3 = plot(sol.t,[Tr_in./(kb*T)],label = L"T_{in,r}")
# p4 = plot(sol.t,[Tr_out./(kb*T)],label = L"T_{out,r}")
# plot(p1,p2,layout = (2,1))
#!!!!!!!!!!!